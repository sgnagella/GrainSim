 ...
 */
#include "BrownianDynamicsPBC.cuh"
...
BaseBrownianIntegrator::BaseBrownianIntegrator(shared_ptr<ParticleGroup> pg,
                                               Parameters par)
    : Integrator(pg, "BD::BaseBrownianIntegrator"), Kx(make_real3(0)),
  ...
  sys->log<System::MESSAGE>("[BD:BaseBrownianIntegrator] Box dimensions: %g %g %g",
  ...
  if (is2D) {
    sys->log<System::MESSAGE>(
        "[BD::BaseBrownianIntegrator] Starting in 2D mode");
  }
...
// Because BD does not depend on velocities, we store the forces in the vel
// for inter-contact calculations 
void BaseBrownianIntegrator::storePastVelocities() {
  int numberParticles = pg->getNumberParticles();
...
real *BaseBrownianIntegrator::getParticleRadiusIfAvailable() {
  ...
  if (hydrodynamicRadius == real(-1.0) && pd->isRadiusAllocated()) {
    ...
    sys->log<System::DEBUG3>(
        "[BD::BaseBrownianIntegrator] Using particle radius.");
  }
  return d_radius;
}
...
void EulerMaruyama::forwardTime() {
  steps++;
  sys->log<System::DEBUG1>("[BD::EulerMaruyama] Performing integration step %d",
                           steps);
  storePastVelocities();
  updateInteractors();
...
void EulerMaruyama::updatePositions() {
  ...
  EulerMaruyama_ns::integrateGPU<<<Nblocks, Nthreads, 0, st>>>(
      pos.raw(), groupIterator, force.raw(), Kx, Ky, Kz, selfMobility, d_radius,
      dt, is2D, temperature, numberParticles, steps, seed, box, image.raw());
}

void MidPoint::forwardTime() {
  steps++;
  sys->log<System::DEBUG1>("[BD::MidPoint] Performing integration step %d",
                           steps);
  updateInteractors();
  computeCurrentForces();
...
void MidPoint::updatePositionsFirstStep() { updatePositions<0>(); }

void MidPoint::updatePositionsSecondStep() { updatePositions<1>(); }

namespace MidPoint_ns {
template <int step>
__global__ void integrateGPU(real4 *pos, real4 *initialPositions,
                             ParticleGroup::IndexIterator indexIterator,
                             const real4 *force, real3 Kx, real3 Ky, real3 Kz,
                             real selfMobility, const real *radius, real dt,
                             bool is2D, real temperature, int N, uint stepNum,
                             uint seed) {
...
template <int step> void MidPoint::updatePositions() {
  int numberParticles = pg->getNumberParticles();
  int BLOCKSIZE = 128;
  uint Nthreads = BLOCKSIZE < numberParticles ? BLOCKSIZE : numberParticles;
  uint Nblocks =
  ...
  MidPoint_ns::integrateGPU<step><<<Nblocks, Nthreads, 0, st>>>(
      pos.raw(), d_initialPositions, groupIterator, force.raw(), Kx, Ky, Kz,
      selfMobility, d_radius, dt, is2D, temperature, numberParticles, steps,
      seed);
  CudaCheckError();
}
...
void AdamsBashforth::forwardTime() {
  ...
  sys->log<System::DEBUG1>(
      "[BD::AdamsBashforth] Performing integration step %d", steps);
  if (steps == 1) {
    updateInteractors();
    computeCurrentForces();
  }
...
void AdamsBashforth::storeCurrentForces() {
  ...
  thrust::copy(thrust::cuda::par.on(st), forceGroup,
               forceGroup + numberParticles, previousForces.begin());
  CudaCheckError();
}

namespace AdamsBashforth_ns {
__global__ void integrateGPU(real4 *pos, real4 *previousForces,
                             ParticleGroup::IndexIterator indexIterator,
                             const real4 *force, real3 Kx, real3 Ky, real3 Kz,
                             real selfMobility, const real *radius, real dt,
                             bool is2D, real temperature, int N, uint stepNum,
...
void AdamsBashforth::updatePositions() {
  ...
  AdamsBashforth_ns::integrateGPU<<<Nblocks, Nthreads, 0, st>>>(
      pos.raw(), d_previousForces, groupIterator, force.raw(), Kx, Ky, Kz,
      selfMobility, d_radius, dt, is2D, temperature, numberParticles, steps,
      seed);
  CudaCheckError();
}
...
__global__ void integrateGPU(real4 *pos,
                             ParticleGroup::IndexIterator indexIterator,
                             const int *originalIndex, const real4 *force,
                             real3 Kx, real3 Ky, real3 Kz, real selfMobility,
                             real *radius, real dt, bool is2D, real temperature,
                             int N, uint stepNum, uint seed) {
  uint id = blockIdx.x * blockDim.x + threadIdx.x;
  if (id >= N)
    return;
  int i = indexIterator[id];
...
void Leimkuhler::forwardTime() {
  steps++;
  sys->log<System::DEBUG1>("[BD::Leimkuhler] Performing integration step %d",
                           steps);
  updateInteractors();
  computeCurrentForces();
  updatePositions();
...
void Leimkuhler::updatePositions() {
  ...
  Leimkuhler_ns::integrateGPU<<<Nblocks, Nthreads, 0, st>>>(
      pos.raw(), groupIterator, originalIndex, force.raw(), Kx, Ky, Kz,
      selfMobility, d_radius, dt, is2D, temperature, numberParticles, steps,
      seed);
}
